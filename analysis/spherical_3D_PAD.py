...
def get_k_sphere(energy, psi, r, l_max, m_max, potential, target, d_angle = 0.01):
    phi = np.arange(-np.pi,np.pi+d_angle,d_angle)
...
for fold in folders:
    ...
    psi = psi[:,0]+1.j*psi[:,1]
    phi, theta, cur_psi = get_k_sphere(e_final, psi, r, l_values.max(), m_values.max(), helium_sae, target)
...
for idx, fold in enumerate(folders):
    with open(fold + '/input.json') as json_file:
        ...
        ax.set_ylabel("y")
        ax.set_zlabel("z")
        plt.title("TDSE - Delay: %.2f \n Cycles: %3.0f    Intensity: %.0e" % (delay,cycles,intensity))
        # plt.show()
