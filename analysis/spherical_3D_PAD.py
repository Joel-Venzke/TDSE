...
def get_k_sphere(energy,
    ...
    r_length = r.shape[0]
    lm_idx = 0
    with open("spherical_harm_TDSE_" + folder + ".txt", "w") as f:
...
def read_data(folders, e_final, r, l_values, m_values, potential, target):
    pad_yield = []
    phi_angles = None
    for fold in folders:
        ...
        phi, theta, cur_psi, phi_angles, d_angle = get_k_sphere(
            e_final, psi, r, l_values.max(), m_values.max(), potential, target,
            fold)
        pad_yield.append(np.abs(cur_psi)**2)

    pad_yield = np.array(pad_yield)
    return pad_yield, phi_angles, theta, phi, d_angle
...
for idx, fold in enumerate(folders):
    with open(fold + '/input.json') as json_file:
        ...
        plt.savefig("PAD_%02.0f_%.1e_%.2f_0p4.png" %
...
for idx, fold in enumerate(folders):
    ...
    with open("Beta_" + fold + ".txt", "w") as f:
        ...
        f.write("\n")
        for l in np.arange(0, beta_max + 1):
            beta_val = np.sum((l + 0.5) * legendre(l)(np.cos(theta)) *
        ...
        for l in np.arange(0, beta_max + 1):
            for m in np.arange(-beta_max, beta_max + 1):
                if np.abs(m) > l:
                    ...
                    beta_val = np.sum(
                        sph_harm(m, l, phi, theta) * cur_data * d_angle *
                        d_angle * np.sin(theta))
                    f.write(str(beta_val) + " ")
            f.write("\n")
...
with open(folders[0] + '/input.json', 'r') as data_file:
    input_file = json.load(data_file)
intensity = input_file["laser"]["pulses"][0]["intensity"]
omega = input_file["laser"]["pulses"][0]["energy"]
U_p = intensity / 3.5e16 / 4. / omega / omega
e_ground = get_energy([1, 0, 0, 1], target)
e_excited = get_energy([2, 1, 1, 1], target)
...
for idx, fold in enumerate(folders):
    with open(fold + '/input.json') as json_file:
        ...
        if np.abs(cycles - 2) < 1e-5 and np.abs(intensity - 1e14) < 1e-5:
            ax.text(1.15, -0.5, 0, "(a)")
        # plt.savefig("PAD_%02.0f_%.1e_%.2f.png" % (cycles, intensity, delay))
        plt.savefig("PAD_%02.0f_%.1e_%.2f_0p4_excited.png" %
                    (cycles, intensity, delay))
...
if e_final > 0:
    ...
    for idx, fold in enumerate(folders):
        with open(fold + '/input.json') as json_file:
            ...
            if np.abs(cycles - 2) < 1e-5 and np.abs(intensity - 1e14) < 1e-5:
                ax.text(1.15, -0.5, 0, "(a)")
            # plt.savefig("PAD_%02.0f_%.1e_%.2f.png" % (cycles, intensity, delay))
            plt.savefig("PAD_%02.0f_%.1e_%.2f_0p4_up.png" %
                        (cycles, intensity, delay))
            # plt.savefig("PAD_%02.0f_%.1e_%.2f_1p0.png" % (cycles, intensity, delay))
            plt.clf()
    ...
    for idx, fold in enumerate(folders):
        # do not normalize to get good cross section
        cur_data = pad_yield[idx]
        with open("Beta_up_" + fold + ".txt", "w") as f:
            f.write("#")
            for l in np.arange(0, beta_max + 1):
                f.write(" beta_%2d" % l)
            f.write("\n")
    ...
    for idx, fold in enumerate(folders):
        cur_data = pad_yield[idx] / max_val
        with open("3D_Beta_up_" + fold + ".txt", "w") as f:
            f.write("#")
            for l in np.arange(0, beta_max + 1):
                f.write(" l_%2d" % l)
            f.write("\n")
...
if e_final > 0:
    ...
    for idx, fold in enumerate(folders):
        with open(fold + '/input.json') as json_file:
            ...
            if np.abs(cycles - 2) < 1e-5 and np.abs(intensity - 1e14) < 1e-5:
                ax.text(1.15, -0.5, 0, "(a)")
            # plt.savefig("PAD_%02.0f_%.1e_%.2f.png" % (cycles, intensity, delay))
            plt.savefig("PAD_%02.0f_%.1e_%.2f_0p4_up_excited.png" %
                        (cycles, intensity, delay))
            # plt.savefig("PAD_%02.0f_%.1e_%.2f_1p0.png" % (cycles, intensity, delay))
            plt.clf()
    ...
    for idx, fold in enumerate(folders):
        # do not normalize to get good cross section
        cur_data = pad_yield[idx]
        with open("Beta_up_excited_" + fold + ".txt", "w") as f:
            f.write("#")
            for l in np.arange(0, beta_max + 1):
                f.write(" beta_%2d" % l)
            f.write("\n")
    ...
    for idx, fold in enumerate(folders):
        cur_data = pad_yield[idx] / max_val
        with open("3D_Beta_up_excited_" + fold + ".txt", "w") as f:
            f.write("#")
            for l in np.arange(0, beta_max + 1):
                f.write(" l_%2d" % l)
            f.write("\n")
