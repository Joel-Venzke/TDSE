...
def get_state(r, energy, l, potential, z=1.0, min_r=500.):
    ...
    for idx in np.arange(2,new_r.shape[0]):
        psi[idx] = psi[idx-1]*(dr*dr*((l*(l+1)/r2[idx-1])+2*potential(new_r[idx-1])-2*energy)+2) - psi[idx-2]
    r_val = new_r[-2]
    psi_r = psi[-2]
    ...
    phase = np.angle((1.j*psi_r + dpsi_r/(k+z/(k*r_val)))/(2*k*r_val)**(1.j*z/k)) -k*r_val + l*np.pi/2
    # phase = 0.0
    ...
    with open("spherical_harm_TDSE_"+fold+".txt","w") as f:
        ...
        for l_val in np.arange(0, l_max + 1):
            ...
            for m_val in np.arange(-m_range, m_range + 1):
                ...
                f.write(str(l_val)+", "+str(m_val)+", "+str(np.abs(coef))+", "+str(np.angle(coef))+"\n")
...
beta_max = 10
detune = float(sys.argv[1].split("_")[-1].strip("/"))

folders = ["cycles_delay_0.00"]
# folders = ["cycles_delay_0.00", "cycles_delay_0.10", "cycles_delay_0.20", "cycles_delay_0.30", "cycles_delay_0.40", "cycles_delay_0.50", "cycles_delay_0.60", "cycles_delay_0.70", "cycles_delay_0.80", "cycles_delay_0.90", "cycles_delay_1.00", "cycles_delay_0.05", "cycles_delay_0.15", "cycles_delay_0.25", "cycles_delay_0.35", "cycles_delay_0.45", "cycles_delay_0.55", "cycles_delay_0.65", "cycles_delay_0.75", "cycles_delay_0.85", "cycles_delay_0.95"]
folders.sort()
...
for fold in folders:
    ...
    psi = psi[:,0]+1.j*psi[:,1]
    phi, theta, cur_psi, phi_angles, d_angle = get_k_sphere(e_final, psi, r, l_values.max(), m_values.max(), helium_sae, target)
...
for idx, fold in enumerate(folders):
    with open(fold + '/input.json') as json_file:
        ...
        cycles =  input_file["laser"]["pulses"][0]["cycles_on"]+input_file["laser"]["pulses"][0]["cycles_off"]
        print(delay,theta[max_idx],phi[max_idx])
        X,Y,Z = cur_data*np.sin(theta)*np.cos(phi),cur_data*np.sin(theta)*np.sin(phi), cur_data*np.cos(theta)
        ...
        ax.set_ylabel("y")
        ax.set_zlabel("z")
        plt.title("TDSE - Detune: %.2f \n Cycles: %6.1f    Intensity: %.0e" % (detune,cycles,intensity))
        # plt.show()
        plt.savefig("PAD_%06.1f_%.1e_%.2f.png" % (cycles,intensity,detune))
        plt.clf()
    ...
    with open("Asym_from_x_"+fold+".txt","w") as f:
        ...
        with open(fold + '/input.json') as json_file:
            ...
            for angle in phi_angles:
                ...
                left_top_sum = np.sum((cur_data[idx_left]*d_angle*d_angle*np.sin(theta[idx_left]))[theta[idx_left]<(np.pi/2)])
                left_bottom_sum = np.sum((cur_data[idx_left]*d_angle*d_angle*np.sin(theta[idx_left]))[theta[idx_left]>=(np.pi/2)])
                right_top_sum = np.sum((cur_data[idx_right]*d_angle*d_angle*np.sin(theta[idx_right]))[theta[idx_right]<(np.pi/2)])
                right_bottom_sum = np.sum((cur_data[idx_right]*d_angle*d_angle*np.sin(theta[idx_right]))[theta[idx_right]>=(np.pi/2)])

                X,Y,Z = np.sin(theta)*np.cos(phi),np.sin(theta)*np.sin(phi), np.cos(theta)


                f.write(str(angle)+", "+str((left_top_sum+left_bottom_sum-(right_top_sum+right_bottom_sum))/(left_top_sum+left_bottom_sum+right_top_sum+right_bottom_sum))+", "+str((left_top_sum+right_bottom_sum-(right_top_sum+left_bottom_sum))/(left_top_sum+left_bottom_sum+right_top_sum+right_bottom_sum))+"\n")
