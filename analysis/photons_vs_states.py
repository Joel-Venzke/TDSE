...
field_max = np.max(np.abs(p["Pulse"]["field_1"][:]))
u_p = field_max * field_max / (c * c * 4.0)

# energies = [
#     -0.507158, -0.125997, -0.125131, -0.0558617, -0.0556058, -0.0555748,
#     -0.0313815, -0.0312736, -0.0312611, -0.0312548, -0.0200681, -0.0200128,
#     -0.0200066, -0.0200033, -0.0200017, -0.0139286, -0.0138966, -0.0138931,
#     -0.0138911, -0.0138902, -0.0138897, -0.0102292, -0.0102091, -0.0102069,
#     -0.0102056, -0.0102051, -0.0102047, -0.0102045, -0.00782939, -0.00781591,
#     -0.00781444, -0.00781362, -0.00781323, -0.007813, -0.00781284, -0.00781272,
#     -0.00618474, -0.00617527, -0.00617425, -0.00617367, -0.0061734,
#     -0.00617323, -0.00617312, -0.00617304, -0.00617297, -0.00500869
# ]
...
if np.max(data) > 1e-10:
    ...
    plt.text(
        spec_time[data_fft[:data_fft.shape[0] / 2].argmax()] + energies[0],
        0.1, str(1))
    plt.semilogy(
        spec_time[:min(spec_time.shape[0], P_1.shape[0])] + energies[0],
        P_1[:min(spec_time.shape[0], P_1.shape[0])] / P_1.max(),
        label=str(1) + " photon")
    spec_max = spec_time[np.argmax(data_fft[:data_fft.shape[0] / 2])]
    n_max = int(
        abs((energies[0] - u_p) / spec_time[np.argmax(
            data_fft[:data_fft.shape[0] / 2])])) + 2
    for n in range(2, n_max):
        print n, "of", n_max - 1
        P_1 = np.convolve(P_1[:data_fft.shape[0] / 2],
                          data_fft[:data_fft.shape[0] / 2])
...
plt.xlim([energies[0], max(energies[-1] + u_p, spec_max)])
plt.ylim([1e-3, 1.0])
# plt.legend()
