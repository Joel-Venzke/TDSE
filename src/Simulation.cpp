  ...
  for (int iter = 0; iter < num_states; iter++)
  {
    ...
    while (!converged)
    {
      ...
      if (i % write_frequency == 0)
      {
        ...
        converged = check_convergance(psi[0], psi_old, parameters->get_tol());
    ...
    // save this psi to ${target}.h5
    wavefunction->checkpoint_psi(states_file, "/States", iter);
    // new Gaussian guess
...
bool Simulation::check_convergance(Eigen::VectorXcd &psi_1,
                                   Eigen::VectorXcd &psi_2, double tol)
{
  Eigen::SparseMatrix<dcomp> *h = hamiltonian->get_time_independent();
...
// gram-schmit to psi
void Simulation::modified_gram_schmidt(std::vector<Eigen::VectorXcd> &states)
{
...
void Simulation::checkpoint_state(HDF5Wrapper &data_file, int write_idx)
{
  wavefunction->normalize();
  wavefunction->checkpoint_psi(data_file, "/States", write_idx);
  data_file.write_object(
      wavefunction->get_energy(hamiltonian->get_time_independent()), "/Energy",
