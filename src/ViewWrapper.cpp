...
void ViewWrapper::WriteAttribute(std::string var_name, std::string name, PetscReal value)
{
    std::string group_path = GetGroup();
    std::cout << group_path << "\n";
    PetscViewerHDF5WriteAttribute(data_file,
        (group_path+var_name).c_str(), name.c_str(),
        PETSC_DOUBLE, &value);
}

void ViewWrapper::WriteAttribute(std::string var_name, std::string name, std::string value)
{
    std::string group_path = GetGroup();
    std::cout << group_path << "\n";
    PetscViewerHDF5WriteAttribute(data_file,
        (group_path+var_name).c_str(), name.c_str(),
        PETSC_STRING, value.c_str());
}
