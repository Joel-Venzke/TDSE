...
class Hamiltonian : protected Utils
{
 private:
  ...
  void CalculateHamlitonian(PetscInt time_idx, PetscInt d_idx);
 ...
 public:
  ...
  Mat *GetTotalHamiltonian(PetscInt time_idx, PetscInt d_idx = -1);
  Mat *GetTimeIndependent(PetscInt d_idx = -1);

  dcomp GetVal(PetscInt idx_i, PetscInt idx_j, bool time_dep, PetscInt time_idx,
               bool &insert_val, PetscInt d_idx);
  ...
  dcomp GetOffDiagonal(std::vector< PetscInt > &idx_array,
                       ...
                       PetscInt time_idx, PetscInt d_idx);
  dcomp GetDiagonal(std::vector< PetscInt > &idx_array, bool time_dep,
                    PetscInt time_idx, PetscInt d_idx);
  dcomp GetKineticTerm(std::vector< PetscInt > &idx_array, PetscInt d_idx);
